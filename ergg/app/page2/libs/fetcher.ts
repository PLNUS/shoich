...
export async function getTraitAll() {  // 버전별로 각각 List 따로 병합하기..
  "use server"
  const res = await fetch("https://stunning-couscous-pjrvwgv55gwphwvq-3000.app.github.dev/api/traitfetcher", {next : {revalidate: 0}});
...
export async function getTSAll() {  // 버전별로 각각 List 따로 병합하기..
  "use server"
  const res = await fetch("https://stunning-couscous-pjrvwgv55gwphwvq-3000.app.github.dev/api/tsfetcher", {next : {revalidate: 0}});
