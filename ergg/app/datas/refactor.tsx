...
export function initData() {
    ...
    Data2.map((char, p) => {
        let newData: Data = {
            ...
            WR: Math.floor(getGameCount([], char.code, 1, 0, 1) / getGameCount([], char.code, 1, 0, 0) * 10000) / 100,
            PR: Math.floor(getGameCount([], char.code, 1, 0, 0) / getGameCount([], 0, 0, 0, 0) * 10000) / 100,
            SR: Math.floor(getSbCount(char.code, 1, 0, 0) / getGameCount([], char.code, 0, 0, 0) * 10000) / 100,
            TK: getAvgTK(char.code, 1, 0, 0),
            avgdeal: Math.floor(getAvgdeal(char.code, 1, 0, 0)),
            SbScore: getSbScore(char.code, 1, 0, 0),
            SbCount: getSbCount(char.code, 1, 0, 0),
        ...
        if (CharMastery[p].weapon2 !== "None") {
            let newData: Data = {
                ...
                WR: Math.floor(getGameCount([], char.code, 2, 0, 1) / getGameCount([], char.code, 2, 0, 0) * 10000) / 100,
                PR: Math.floor(getGameCount([], char.code, 2, 0, 0) / getGameCount([], 0, 0, 0, 0) * 10000) / 100,
                SR: Math.floor(getSbCount(char.code, 2, 0, 0) / getGameCount([], char.code, 0, 0, 0) * 10000) / 100,
                TK: getAvgTK(char.code, 2, 0, 0),
                avgdeal: Math.floor(getAvgdeal(char.code, 2, 0, 0)),
                SbScore: getSbScore(char.code, 2, 0, 0),
                SbCount: getSbCount(char.code, 2, 0, 0),
        ...
        if (CharMastery[p].weapon3 !== "None") {
            let newData: Data = {
                ...
                WR: Math.floor(getGameCount([], char.code, 3, 0, 1) / getGameCount([], char.code, 3, 0, 0) * 10000) / 100,
                PR: Math.floor(getGameCount([], char.code, 3, 0, 0) / getGameCount([], 0, 0, 0, 0) * 10000) / 100,
                SR: Math.floor(getSbCount(char.code, 3, 0, 0) / getGameCount([], char.code, 0, 0, 0) * 10000) / 100,
                TK: getAvgTK(char.code, 3, 0, 0),
                avgdeal: Math.floor(getAvgdeal(char.code, 3, 0, 0)),
                SbScore: getSbScore(char.code, 3, 0, 0),
                SbCount: getSbCount(char.code, 3, 0, 0),
        ...
        if (CharMastery[p].weapon4 !== "None") {
            let newData: Data = {
                ...
                WR: Math.floor(getGameCount([], char.code, 4, 0, 1) / getGameCount([], char.code, 4, 0, 0) * 10000) / 100,
                PR: Math.floor(getGameCount([], char.code, 4, 0, 0) / getGameCount([], 0, 0, 0, 0) * 10000) / 100,
                SR: Math.floor(getSbCount(char.code, 4, 0, 0) / getGameCount([], char.code, 4, 0, 0) * 10000) / 100,
                TK: getAvgTK(char.code, 4, 0, 0),
                avgdeal: Math.floor(getAvgdeal(char.code, 4, 0, 0)),
                SbScore: getSbScore(char.code, 4, 0, 0),
                SbCount: getSbCount(char.code, 4, 0, 0),
...
function getNadjaPoint(char: Data) { // 티어 산출 밸런싱 필요
    ...
    let var2: number = (char.SbScore * char.PR) > 0 ? (char.SbScore * char.PR) : 0;
    let var3: number = char.TK;
    let var4: number = char.WR;
    let var5: number = char.avggrade;
    return (var1) / 1000 + (var2) / 500 + var3 * 10 + var4 * 10 - var5 * 5;
}
...
function getSbCount(code: number, weapon: number, tiergroup: number, grade: number) {
    // 순방 수 반환
    // code, weapon 은 0일 수 없음
    let count = 0;
...
function getSbScore(code: number, weapon: number, tiergroup: number, grade: number) {
    // 순방 점수 반환
    // code, weapon 은 0일 수 없음
    let score = 0;
